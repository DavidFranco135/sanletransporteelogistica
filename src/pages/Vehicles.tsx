...
import { getVehicles, createVehicle } from '../services/firebaseService';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Buscar veículo por modelo ou placa..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
        ...
        {filtered.map((vehicle) => (
          <div key={vehicle.id} className="bg-[#1e293b] rounded-2xl border border-slate-800 shadow-xl overflow-hidden hover:shadow-2xl transition-all group">
            ...
            <div className="p-6">
              ...
              {vehicle.notes && <p className="text-xs text-slate-400 bg-[#0f172a] p-3 rounded-lg italic border border-slate-800">{vehicle.notes}</p>}
            </div>
          </div>
        ))}
      </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative h-32 border-2 border-dashed border-slate-700 rounded-xl flex flex-col items-center justify-center text-slate-500 hover:border-emerald-500 hover:text-emerald-500 transition-all cursor-pointer bg-[#0f172a]">
                    ...
                    {photo ? <span className="text-emerald-400 font-bold">{photo.name}</span> : <><Camera size={32} /><span className="text-xs mt-2">Clique para selecionar</span></>}
                  </div>
                </div>
