...
// Carrega logo com dimensões naturais para manter proporção
...
// Insere logo no PDF mantendo proporção dentro de uma caixa
...
export const generateGeneralReportPDF = async (title: string, data: (string|number)[][], columns: string[]) => {
  ...
  if (data.length===0) { doc.setFontSize(11); doc.setTextColor(100,116,139); doc.text('Nenhum registro encontrado.', margin, y); }
  else { y = addSectionTitle(doc, 'LISTAGEM DE REGISTROS', y); drawTable(doc, { startY:y, headers:columns, rows:data, colWidths:colW, rowHeight:9, fontSize:9, marginL:margin }); }
